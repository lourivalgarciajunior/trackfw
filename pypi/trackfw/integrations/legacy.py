"""Byte-exact hashes emitted by released pre-lifecycle installers."""
...
LEGACY_HASHES: dict[str, tuple[str, ...]] = {
...
def legacy_hashes(claim: dict[str, Any]) -> list[str]:
    key = ":".join(str(claim[field]) for field in ("target", "surface", "scope", "kind", "item"))
    return list(LEGACY_HASHES.get(key, ()))
