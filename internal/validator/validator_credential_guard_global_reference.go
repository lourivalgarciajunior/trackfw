...
// internal/generators/scaffold.go (globalCredentialGuardScript = credentialGuardHeader +
// credentialGuardDetectionCore + credentialGuardGlobalTail). This is a DIFFERENT template than
// credentialGuardScriptReference (the project-scope variant): the global variant omits the
// project-guard block ("no-op outside a trackfw.yaml project") and defaults
// credential_guard.mode to "block" instead of "warn" — see GenerateGlobalCredentialGuardScript's
// doc comment in scaffold.go for the full rationale. Comparing the global on-disk script against
// credentialGuardScriptReference (the project template) would be a guaranteed false positive for
// every user with the global harness installed — this repo included (ROADMAP-2026-08-15-trackfw-
// validate-deve-detectar-scripts-de-hook-ausentes-ou-desatualizados, ML-1A).
//
