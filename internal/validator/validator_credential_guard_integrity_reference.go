...
// credentialGuardDetectionCore + credentialGuardProjectTail). internal/validator cannot import
// internal/generators directly: generators/context.go already imports validator (for `trackfw
// context`), so importing generators from validator would create a Go import cycle. This ML
// (ROADMAP-2026-08-12-deteccao-de-adulteracao-do-credential-guard-regra-de-validate, ML-1A) is
// also scoped OUT of internal/generators/ (ML-2A/ML-3A own that layer), so exporting a
// string-returning variant there is not an option either.
//
...
// against this constant. That test is materially stronger than
// TestCredentialGuardScript_ParityAcrossStacks (internal/generators/credential_guard_test.go),
// which never executes the runtimes it compares (see ADR-2026-08-12 Emenda 2) — this one executes
// the real Go generator.
